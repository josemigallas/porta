...
When "I try to {action} the active docs with {valid} data" do |action, valid|
...
When "I try to {action} the active docs of the service with {valid} data" do |action, valid|
  action_page = action == 'update' ? 'edit' : 'new'
  step %(I go to the #{action_page} active docs page for a service)
  step %(I #{action} the action docs with #{valid ? 'valid' : 'invalid'})
end

When "I {action} the active docs with {valid} data" do |action, valid|
