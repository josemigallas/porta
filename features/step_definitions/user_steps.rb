...
Given "an user {string} of account {account}" do |username, account|
...
Given "a pending user {string} of account {account}" do |username, account|
  FactoryBot.create(:pending_user, account: account, username: username)
end

Given "an active user {string} of account {account}" do |username, account|
  FactoryBot.create(:active_user, account: account, username: username)
end

Given "an active admin {string} of account {account}" do |username, account|
  FactoryBot.create(:active_admin, account: account, username: username)
end

Given "an active user {string} of account {account} with {string} permission" do |username, account, permission|
...
Given "an active user {string} of account {account} with email {string}" do |username, account, email|
...
Given "the admin of account {account} has password {string}" do |account, password|
  user = account.admins.first
  user.password = password
  user.save!
end

Given "the admin of account {account} has email {string}" do |account, email|
  user = account.admins.first
  user.email = email
  user.save!
end
...
Given "provider {string} has user {string}" do |org_name, username|
  provider = Account.find_by!(org_name: org_name)
...
Then "there should be no user with username {string} of account {account}" do |username, account|
  assert_nil account.users.find_by(username: username)
end

Then "user {user} should have role {string}" do |user, role|
  assert_equal role.to_sym, user.role
end

Then "user {user} should be {state}" do |user, state|
  assert user.send("#{state}?")
end

Then "user {user} should have last login on {} from {}" do |user, time, ip|
