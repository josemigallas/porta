...
When "I (follow/press) {string} for {metric}" do |action, label, metric|
  binding.pry # check action
...
Then "I should see metric {string}" do |name|
...
Then "I should not see metric {string}" do |name|
  metrics = XPath.anywhere[XPath.attr(:id).equals('metrics')]
  selector = metrics.descendant(:td)[XPath.text.contains(name)]
  assert_no_selector :xpath, selector
end
