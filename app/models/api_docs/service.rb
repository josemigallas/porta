...
class ApiDocs::Service < ApplicationRecord
  ...
  before_validation(on: :create) { self.account ||= owner_account }

  scope :published, -> { where(published: true) }
  scope :owned_by_backend_api, -> { where(owner_type: 'BackendApi') }
  scope :service_accessible, -> { joining { service.outer }.where.has { (service_id == nil) | (service.state != ::Service::DELETE_STATE) } }
  ...
  def self.for(account)
    services = account.all_api_docs.published
  ...
  def service_belongs_to_account
    object_service_id = service_id.presence
    object_service_id ||= owner_id if owner_type == 'Service'

    return true if account.services.accessible.where(id: object_service_id).exists?
